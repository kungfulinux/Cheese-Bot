  ...
  if (haveCredentials) {
    ...
    if (options.session) {
      yggdrasil.validate(options.session.accessToken, function (ok) {
        if (ok) cb(null, options.session);else yggdrasil.refresh(options.session.accessToken, options.session.clientToken, function (err, _, data) {
          cb(err, data);
        });
      });
    } else yggdrasil.auth({
  ...
  function onEncryptionKeyRequest(packet) {
    ...
    function gotSharedSecret(err, sharedSecret) {
      ...
      function joinServerRequest(cb) {
        yggserver.join(accessToken, client.session.selectedProfile.id, packet.serverId, sharedSecret, packet.publicKey, cb);
      }
...
//# sourceMappingURL=maps/createClient.js.map
